-- ════════════════════════════════════════════════════════════════════════════
--  Horodatage de chaque étape vecteur + Sur place personnel (première heure).
--  Idempotent. Après 35_stock_o2_bouteilles.sql.
-- ════════════════════════════════════════════════════════════════════════════

create or replace function public._marquer_heure_etape(m jsonb, vid text, etape text, iso text)
returns jsonb language plpgsql immutable as $$
begin
  if vid is null or vid = '' or etape is null or etape = '' then
    return coalesce(m, '{}'::jsonb);
  end if;
  if coalesce(m #>> array['vecteur_etape_heures', vid, etape], '') <> '' then
    return coalesce(m, '{}'::jsonb);
  end if;
  return public._jsonb_set_path(coalesce(m, '{}'::jsonb), array['vecteur_etape_heures', vid, etape], to_jsonb(iso));
end $$;
...
grant execute on function public._marquer_heure_etape(jsonb, text, text, text) to authenticated;
