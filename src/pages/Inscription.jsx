...
export default function Inscription() {
  ...
  useEffect(() => {
    ...
    supabase.rpc('verifier_invitation', { p_code: codeLien, p_email: emailLien }).then(({ data }) => {
      if (stop) return
      if (!data?.ok) { setErr('Cette invitation est invalide, déjà utilisée ou expirée.'); setInviteInvalide(true) }
      else { setInvite(data); setInviteInvalide(false) }
    })
    return () => { stop = true }
  }, [codeLien, emailLien])
